...
package org.thunderdog.challegram.animator;
...
public final class ListAnimator<T> implements Iterable<ListAnimator.Entry<T>> {
  ...
  public void measure (boolean animated) {
    measure(animated, Integer.MAX_VALUE, 0, Integer.MAX_VALUE);
  }

  public void measure (boolean animated, @Px int maxWidth, int minItemsInRow, int maxItemsInRow) {
    if (!animated) {
      stopAnimation(true);
    }
    measureImpl(animated, maxWidth, minItemsInRow, maxItemsInRow);
    if (animated) {
      onApplyListChanges();
    }
  }

  private void measureImpl (boolean animated, @Px int maxWidth, int minItemsInRow, int maxItemsInRow) {
    int itemsInRow = 0;
    ...
    for (Entry<T> entry : actualList) {
      if (entry.item instanceof Measurable) {
        ...
        // FIXME sorry
        if (maxWidth < Integer.MAX_VALUE || maxItemsInRow < Integer.MAX_VALUE) {
          if ((lineWidth + width > maxWidth && itemsInRow >= minItemsInRow) || itemsInRow >= maxItemsInRow) {
  ...
  public void reset (@Nullable List<T> newItems, boolean animated, @Nullable ResetCallback<T> resetCallback) {
    reset(newItems, animated, resetCallback, Integer.MAX_VALUE);
  }

  public void reset (@Nullable List<T> newItems, boolean animated, @Nullable ResetCallback<T> resetCallback, @Px int maxWidth) {
    reset(newItems, animated, resetCallback, maxWidth, 0, Integer.MAX_VALUE);
  }

  public void reset (@Nullable List<T> newItems, boolean animated, @Nullable ResetCallback<T> resetCallback, @Px int maxWidth, int minItemsInRow, int maxItemsInRow) {
    if (!animated) {
      stopAnimation(false);
      for (int i = entries.size() - 1; i >= 0; i--) {
      ...
      measureImpl(false, maxWidth, minItemsInRow, maxItemsInRow);
    ...
    measureImpl(true, maxWidth, minItemsInRow, maxItemsInRow);
