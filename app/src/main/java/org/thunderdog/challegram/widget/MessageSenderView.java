...
public class MessageSenderView extends BaseView implements Destroyable, RemoveHelper.RemoveDelegate, ChatListener, TdlibCache.UserDataChangeListener, TdlibCache.SupergroupDataChangeListener, TdlibCache.BasicGroupDataChangeListener, NotificationSettingsListener, TdlibCache.UserStatusChangeListener, DrawableProvider, TooltipOverlayView.LocationProvider {
  ...
  private TGMessageSender messageSender;

  public MessageSenderView (Context context, Tdlib tdlib) {
    super(context, tdlib);
  ...
  protected void onDraw (Canvas c) {
    ...
    if (messageSender.isSelected()) {
      Drawable selectedIcon = Drawables.get(R.drawable.baseline_check_circle_blue_24);
      Paint paint = Paints.fillingPaint(Theme.fillingColor());
      c.drawCircle(receiver.getCenterX() + Screen.dp(17f), receiver.getCenterY() + Screen.dp(17f), Screen.dp(12f), paint);
      Drawables.draw(c, selectedIcon, receiver.getCenterX() + Screen.dp(5f), receiver.getCenterY() + Screen.dp(5f), Paints.getBitmapPaint());
    }
    ...
    Drawable finishIcon = null;
    if (messageSender.isAnonymousAdmin()) {
  ...
  private TGFoundChat lastChat1;
  private TGMessageSender lastChat;
...
//    }
  ...
  public void setMessageSender (TGMessageSender messageSender) {
    this.messageSender = messageSender;
    setTitle(messageSender.getTitle());
    setSubtitle(messageSender.getSubtitle());
    setAvatar(messageSender.getAvatar(), messageSender.getAvatarPlaceholderMetadata());
  }
