    ...
    if (displayTitle != null) {
      ...
      if (!isSelf && (isVerified || isPremium)) {
        ...
        if (isVerified) {
          ...
          Drawables.drawRtl(c, drawable, displayTitle.getWidth() + titleLeft + addedIconsWidth * (rtl ? -1 : 1), Screen.dp(12f), Paints.getVerifyPaint(), width, rtl);
          addedIconsWidth += drawable.getMinimumWidth();
        }
        if (isPremium) {
          Drawable drawable = Icons.getChatPremiumDrawable();
          Drawables.drawRtl(c, drawable, displayTitle.getWidth() + titleLeft + addedIconsWidth * (rtl ? -1 : 1), Screen.dp(11.5f), Paints.getPremiumPaint(), width, rtl);
        }
      }
    }
