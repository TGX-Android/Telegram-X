    ...
    public boolean uncapture(MotionEvent event, int pointerIndex) {
        if (pointerIndex == 0) {
            if (tryMoveTo) {
                ...
                if (event.getAction() == MotionEvent.ACTION_UP && System.currentTimeMillis() - startTapTime < 300 && Math.sqrt(dx * dx + dy * dy) < Screen.dp(10)) {

                    float moveToX = (this.moveToX - BaseChartView.getHorizontalPadding()) / pickerWidth;
