...
import org.thunderdog.challegram.charts.data.StackLinearChartData;
...
import org.thunderdog.challegram.charts.view_data.PieLegendView;
...
public class PieChartView extends StackLinearChartView<PieChartViewData> {
    ...
    float MIN_TEXT_SIZE = Screen.dp(9);
    float MAX_TEXT_SIZE = Screen.dp(13);
    ...
    protected void drawChart(Canvas canvas) {
        ...
        rectF.set(
                ...
                chartArea.centerY() + Screen.dp(16) - radius,
                chartArea.centerX() + radius,
                chartArea.centerY() + Screen.dp(16) + radius
        );
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (lines.get(i).selectionA > 0f) {
                ...
                canvas.translate(
                        (float) (Math.cos(Math.toRadians(textAngle)) * Screen.dp(8) * ai),
                        (float) (Math.sin(Math.toRadians(textAngle)) * Screen.dp(8) * ai)
                );
            }
    ...
    protected void selectXOnChart(int x, int y) {
        ...
        double theta = Math.atan2(chartArea.centerY() + Screen.dp(16) - y, chartArea.centerX() - x);
