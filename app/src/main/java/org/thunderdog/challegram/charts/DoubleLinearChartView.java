    ...
    protected void drawChart(Canvas canvas) {
        if (chartData != null) {
            ...
            for (int k = 0; k < lines.size(); k++) {
                ...
                int additionalPoints = (int) (getHorizontalPadding() / p) + 1;
                ...
                for (int i = localStart; i <= localEnd; i++) {
                    ...
                    float yPoint = getMeasuredHeight() - chartBottom - padding - (yPercentage) * (getMeasuredHeight() - chartBottom - getSignatureTextHeight() - padding);
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (a.valuesStr != null && lines.size() > 0) {
                if (a.valuesStr2 == null || lines.size() < 2) {
                    signaturePaint.setColor(Theme.textDecentColor()); // TODO key_statisticChartSignature
                    signaturePaint.setAlpha((int) (a.alpha * signaturePaintAlpha * transitionAlpha * additionalOutAlpha));
                } else {
                    LineViewData line = lines.get(leftIndex);
                    line.lineColor = Theme.textDecentColor();
                    signaturePaint.setColor(line.lineColor);
                    signaturePaint.setAlpha((int) (a.alpha * line.alpha * transitionAlpha * additionalOutAlpha));
                }

                canvas.drawText(a.valuesStr[i], getHorizontalPadding(), y - textOffset, signaturePaint);
            }
            if (a.valuesStr2 != null && lines.size() > 1) {
                LineViewData line = lines.get(rightIndex);
                line.lineColor = Theme.textDecentColor();
                signaturePaint2.setColor(line.lineColor);
                signaturePaint2.setAlpha((int) (a.alpha * line.alpha * transitionAlpha * additionalOutAlpha));
                canvas.drawText(a.valuesStr2[i], getMeasuredWidth() - getHorizontalPadding(), y - textOffset, signaturePaint2);
            }
        }
    }
