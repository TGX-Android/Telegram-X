...
public class SettingsThemeController extends RecyclerViewController<SettingsThemeController.Args> implements View.OnClickListener, ViewController.SettingsIntDelegate, SliderWrapView.RealTimeChangeListener, View.OnLongClickListener, TGLegacyManager.EmojiLoadListener, AppUpdater.Listener, TdlibSettingsManager.QuickReactionChangeListener {
  ...
  public void destroy () {
    ...
    tdlib().settings().removeQuickReactionChangeListener(this);
  ...
  protected void onCreateView (Context context, CustomRecyclerView recyclerView) {
      ...
      protected void setValuedSetting (ListItem item, SettingView v, boolean isUpdate) {
        ...
        switch (item.getId()) {
          ...
          case R.id.btn_quickReaction: {
            ...
            if (!TdlibSettingsManager.QUICK_REACTION_DISABLED.equals(quickReaction)) {
              TdApi.Reaction reaction = tdlib().getReaction(quickReaction);
              v.setDrawModifier(new ReactionModifier(tdlib(), reaction));
              if (reaction != null) {
              ...
              v.setData(Lang.getString(R.string.QuickReactionDisabled));
          ...
          case R.id.btn_useBigReactions: {
            StringBuilder b = new StringBuilder();
            if (tdlib().settings().useBigReactionsInChats()) {
              b.append(Lang.getString(R.string.Chats));
            }
            if (tdlib().settings().useBigReactionsInChannels()) {
              if (b.length() > 0) {
                b.append(Lang.getConcatSeparator());
              }
            ...
            if (b.length() == 0) {
              b.append(Lang.getString(R.string.BigReactionsOff));
            }
            v.setData(b.toString());
            break;
          }
    ...
    if (mode == MODE_THEMES) {
      ...
      tdlib().settings().addQuickReactionChangeListener(this);
      items.add(new ListItem(ListItem.TYPE_SEPARATOR_FULL));
      items.add(new ListItem(ListItem.TYPE_VALUED_SETTING, R.id.btn_quickReaction, 0, R.string.QuickReaction));
  ...
  @Override
  public void onQuickReactionChanged (String reaction) {
    if (adapter != null) {
  ...
  public void onClick (View v) {
    switch (v.getId()) {
      ...
      case R.id.btn_useBigReactions: {
        showSettings(new SettingsWrapBuilder(R.id.btn_useBigReactions).setRawItems(new ListItem[]{
          new ListItem(ListItem.TYPE_CHECKBOX_OPTION, R.id.btn_chats, 0, R.string.Chats, R.id.btn_chats, tdlib().settings().useBigReactionsInChats()),
          new ListItem(ListItem.TYPE_CHECKBOX_OPTION, R.id.btn_channels, 0, R.string.Channels, R.id.btn_channels, tdlib().settings().useBigReactionsInChannels())
        }).setIntDelegate((id, result) -> {
          tdlib().settings().setUseBigReactionsInChats(result.get(R.id.btn_chats) == R.id.btn_chats);
          tdlib().settings().setUseBigReactionsInChannels(result.get(R.id.btn_channels) == R.id.btn_channels);
          adapter.updateValuedSettingById(R.id.btn_useBigReactions);
        }).setAllowResize(false).addHeaderItem(Lang.getString(R.string.BigReactionsInfo)));
        break;
      }
      case R.id.btn_systemEmoji: {
