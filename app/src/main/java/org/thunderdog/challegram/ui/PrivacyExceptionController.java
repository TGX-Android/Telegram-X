        ...
        switch (item.getId()) {
          case R.id.btn_privacyRule: {
            ...
            if (PrivacySettings.isGeneral(matchingRule, !isMultiChat)) {
              ...
              view.setDataColorId(R.id.theme_color_textNeutral);
              switch (setting.getConstructor()) {
                case TdApi.UserPrivacySettingShowPhoneNumber.CONSTRUCTOR:
          ...
          case R.id.btn_newContact: {
            ...
            view.setTextColorId(item.getTextColorId(R.id.theme_color_text));
            view.setIcon(item.getIconResource());
            if (!isMultiChat) {
              view.setIconColorId(tdlib.cache().userContact(userId) ? R.id.theme_color_iconNegative : 0);
            }
            break;
          }
        }
