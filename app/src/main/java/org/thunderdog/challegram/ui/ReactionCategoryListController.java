...
public class ReactionCategoryListController extends ViewController<ReactionsLayout> {
  ...
  protected View onCreateView (Context context) {
    ...
    for (int i = 0; i < reactions.length; i++) {
      ...
      items.add(new ReactionCategoryListController.Item(reaction, reactionColorState));
    }
  ...
  private static class ReactionAdapter extends RecyclerView.Adapter<ReactionCategoryListController.ItemHolder> {
    private static final int REACTION_SIZE = Screen.dp(60f);

    private final Context context;
    private final List<ReactionCategoryListController.Item> items;
    ...
    public ReactionAdapter (
        Context context,
        List<ReactionCategoryListController.Item> items,
    ...
    @NonNull
    @Override
    public ReactionCategoryListController.ItemHolder onCreateViewHolder (@NonNull ViewGroup parent, int viewType) {
      ...
      return new ReactionCategoryListController.ItemHolder(reactionView);
    }

    @Override
    public void onBindViewHolder (@NonNull ReactionCategoryListController.ItemHolder holder, int position) {
      ReactionCategoryListController.Item item = items.get(position);
