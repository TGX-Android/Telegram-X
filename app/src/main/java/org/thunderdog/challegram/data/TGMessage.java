...
import org.thunderdog.challegram.component.thread.TGCommentButton;
...
public abstract class TGMessage implements InvalidateContentProvider, TdlibDelegate, FactorAnimator.Target, Comparable<TGMessage>, Counter.Callback {
  ...
  protected TGMessage (MessagesManager manager, TdApi.Message msg) {
    ...
    this.commentButton.setForceDisplayEndIcon(!useBubbles());
    this.commentButton.setNeedBackground(useBubbles() && getCommentMode() == COMMENT_MODE_DETACHED_BUTTON);
  ...
  private int computeBubbleWidth () {
    ...
    if (allowMessageHorizontalExtend()) {
      ...
      if (needCommentButton()) {
        width = Math.max(width, commentButton.getPreferredMinWidth());
      }
    }
  ...
  private int computeBubbleHeight () {
    ...
    if (needCommentButton() && !useStickerBubbleReactions()) {
  ...
  public void buildLayout (int width) {
    ...
    if (!useBubbles()) buildCommentButton(false);
  ...
  public final void draw (MessageView view, Canvas c, @NonNull ImageReceiver avatarReceiver, Receiver replyReceiver, ComplexReceiver replyTextMediaReceiver, DoubleImageReceiver previewReceiver, ImageReceiver contentReceiver, GifReceiver gifReceiver, ComplexReceiver complexReceiver) {
    ...
    // Reaction bubbles
    if (useReactionBubbles) {
      int commentButtonOffset = needCommentButton() && !useMediaBubbleReactions() ? getCommentButtonHeight() : 0;
      int top = (int) (this.height - messageReactions.getAnimatedHeight() - getExtraPadding() - commentButtonOffset);
      if (!useBubbles) {
        ...
        if (useMediaBubbleReactions()) {
          ...
          drawReactionsWithBubbles(c, view, (int) bubblePathRect.left + xReactionBubblePadding, (bottomContentEdge - (int) messageReactions.getAnimatedHeight() - timeAddedHeight - xReactionBubblePaddingBottom - commentButtonOffset));
        }
      }
    }
  ...
  protected void drawBubbleTimePart (Canvas c, MessageView view) {
    ...
    int offset = needCommentButton() && !useStickerBubbleReactions() ? getBubbleReduceHeight() : 0;
    int startY = bottomContentEdge - getBubbleTimePartHeight() - getBubbleTimePartOffsetY() - offset;
  ...
  private void buildCommentButton(boolean animated) {
    if (!needCommentButton()) return;
    int bottom = this.bottomContentEdge;
    if ((useStickerBubbleReactions() || !useBubble())) {
      bottom = findBottomEdge();
    ...
    commentButton.setBounds(w, leftContentEdge, bottom - getBubbleReduceHeight(), animated);
