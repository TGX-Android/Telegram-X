...
public abstract class TGMessage implements MultipleViewProvider.InvalidateContentProvider, TdlibDelegate, FactorAnimator.Target, Comparable<TGMessage>, Counter.Callback {
  ...
  private int measureReactionsHeight () {
    return reactionsComponent != null ? reactionsComponent.getHeight() : 0;
  }
  ...
  public final void draw (MessageView view, Canvas c, @NonNull ImageReceiver avatarReceiver, Receiver replyReceiver, DoubleImageReceiver previewReceiver, ImageReceiver contentReceiver, GifReceiver gifReceiver, ComplexReceiver complexReceiver) {
    ...
    if (useForward()) {
      ...
      if (useBubbles) {
        lineTop = forwardY;
        lineBottom = bottomContentEdge - xBubblePadding - xBubblePaddingSmall - (useBubbleTime() ? getBubbleTimePartHeight() : 0) - getBubbleReduceHeight() - ((reactionsComponent != null && !reactionsComponent.shouldRenderSmall() && !reactionsComponent.shouldRenderUnderBubble()) ? reactionsComponent.getHeight(isBubbleTimeExpanded) : 0);
  ...
  protected final void buildBubble (boolean force) {
    if (useBubbles()) {
      ...
      int reactionsHeight = (reactionsComponent != null && !reactionsComponent.shouldRenderSmall() && !reactionsComponent.shouldRenderUnderBubble()) ? reactionsComponent.getHeight() : 0;
  ...
  public void translateVertical (float dy) {
    ...
    translationYLockAnimator.setInterpolator(useBubbles() ? AnimatorUtils.OVERSHOOT_INTERPOLATOR : AnimatorUtils.ACCELERATE_DECELERATE_INTERPOLATOR);
    translationYLockAnimator.setDuration(useBubbles() ? 210l : 140l);
  ...
  private void drawTranslate (View view, Canvas c) {
    ...
    if (x > 0) {
      ...
      if (shouldRenderReactions) {
        c.save();
        c.translate(0, blockHeight * tsFactorInv);

        c.drawRect(endX + x, startY, endX, endY, Paints.fillingPaint(ColorUtils.alphaColor(tsFactor, quickColor)));
        if (check > height) {
          c.save();
          float scale = (float) height / (float) check;
          c.scale(scale, scale, iconX, cy);
        ...
        int ccx = (int) iconX + Screen.dp(12f);
        int w = icon.getMinimumWidth() / 2;
        int h = icon.getMinimumHeight() / 2;
        iQuickReaction.setBounds((int) ccx - w, cy - h, (int) ccx + w, cy + h);
        ...
        c.drawText(quickReactionText, textX, cy + xQuickTextOffset, mQuickText);
