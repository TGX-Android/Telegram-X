  ...
  private void buildHeader (TdApi.WebPage webPage, int maxWidth) {
    ...
    if (!StringUtils.isEmpty(webPage.siteName) || isTgWallpaper()) {
      ...
      siteName = new Text.Builder(actualSiteName, maxWidth, TGMessage.getTextStyleProvider(), parent.getChatAuthorColorSet())
        ...
        .clipTextArea()
        .entities(isTgWallpaper() ? new TextEntity[]{} : new TextEntity[]{TextEntity.valueOf(parent.tdlib, webPage.siteName, new TdApi.TextEntity(0, webPage.siteName.length(), new TdApi.TextEntityTypeTextUrl(url)), parent.openParameters())})
        .build();
