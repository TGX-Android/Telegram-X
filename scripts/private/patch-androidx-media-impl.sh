...
for FLAVOR in ${flavors[@]}; do
  ...
  files=( ffmpeg_jni flac_jni flac_parser opus_jni vpx_jni )
  for file in "${files[@]}"
  do
    echo "Patching $file..."
    extra_rule="\$!N;s/(^jint JNI_(OnLoad\(JavaVM *\* *[a-zA-Z0-9_]*, void *\* *[a-zA-Z0-9_]*\) \{\n(  [^\n]+\n)*\}))/extern \"C\" jint ${file}_\2/g;P;D"
    extra_rules="${sed_rules}${extra_rule}"
    $SED -i".bak" -E "$extra_rules" "$file.cc" && rm "$file.cc.bak"
  done
