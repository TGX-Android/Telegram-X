...
@CacheableTask
abstract class GenerateEmojiSetsTask : DefaultTask() {
  @get:InputFile
  @get:PathSensitive(PathSensitivity.RELATIVE)
  abstract val emojiCode: RegularFileProperty

  @get:InputFile
  @get:PathSensitive(PathSensitivity.RELATIVE)
  abstract val emojiCodeColored: RegularFileProperty
  ...
  fun checkEmojiKeyboard () {
    ...
    val supportedFile = emojiCode.get().asFile.readText()
    val displayingFile = emojiCodeColored.get().asFile.readText()
    ...
    writeToFile(
      kotlin.resolve("org/thunderdog/challegram/tool/Emojis.kt")
    ) { kt ->
      kt.append("""
        @file:JvmName("Emojis")
    ...
    for (i in 0 .. 1) {
      ...
      writeToFile(
        kotlin.resolve("org/thunderdog/challegram/tool/EmojiBidi${legacySuffix}.kt")
      ) { kt ->
        kt.append("""
